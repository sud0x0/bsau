...
func newOllamaProvider(baseURL, model string, maxFileBytes int) *ollamaProvider {
...
func (p *ollamaProvider) CheckAvailability() error {
...
func (p *ollamaProvider) AnalyzeFormula(pkg string, versions []FormulaVersion) (*FormulaAnalysisResult, error) {
	...
	if len(versions) == 1 {
		...
		response, err := p.sendChatRequest(FormulaSystemPrompt, userPrompt)
		if err != nil {
			result.Error = err
...
func (p *ollamaProvider) AnalyzeCode(pkg, oldVersion, newVersion, diff, yaraFindings string) (*CodeAnalysisResult, error) {
	...
	if len(diffLines) <= DiffChunkSize {
		// Small diff - analyze in one request, include YARA findings
...
func (p *ollamaProvider) analyzeDiffChunk(pkg, oldVersion, newVersion, diffChunk, yaraFindings string, startLine, endLine int, includeYara bool, chunkNum, totalChunks int, noteTruncation bool, result *CodeAnalysisResult) (*CodeAnalysisResult, error) {
	var content strings.Builder
	fmt.Fprintf(&content, "Package: %s\n", pkg)
	...
	for attempt := 1; attempt <= MaxRetries; attempt++ {
		response, lastErr = p.sendChatRequest(DiffSystemPrompt, userPrompt)
		if lastErr == nil {
			break
		}
...
func (p *ollamaProvider) AnalyzeFiles(pkg string, files map[string]string, yaraFindings string) (*CodeAnalysisResult, error) {
	...
	for filePath, content := range files {
		...
		for i, chunk := range chunks {
			...
			response, err := p.sendChatRequest(SystemPrompt, userPrompt)
			if err != nil {
...
func (p *ollamaProvider) sendChatRequest(systemPrompt, userPrompt string) (string, error) {
	req := ollamaChatRequest{
		Model: p.model,
		Messages: []ollamaChatMessage{
	...
	// Log request
	logger.LLMRequest("ollama", p.model, systemPrompt, userPrompt)
	...
	if resp.StatusCode != http.StatusOK {
		...
		logger.LLMError("ollama", fmt.Errorf("status %d: %s", resp.StatusCode, string(errBody)))
	...
	if response.Message.Content == "" {
		logger.LLMError("ollama", fmt.Errorf("empty response"))
		return "", fmt.Errorf("empty response from ollama")
	}

	// Log response
	logger.LLMResponse("ollama", elapsed, response.Message.Content)
